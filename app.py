...
def dashboard():
    ...
    for ques in daily_quiz:
        ...
        all_options=[]
        for opt in options:
            all_options.append({"id":opt[0],"text":opt[1]})
...
def check_answer():
    ...
    if correct==answer:
        cursor.execute("""
            UPDATE attempts SET score = score + 10
            WHERE user_id=%s AND quiz_id=1
        """, (session['user_id'],))
        cursor.execute("""
            UPDATE users SET total_score = total_score + 10
            WHERE user_id=%s
            """,(session["user_id"],))
        db.commit()
        ...
        cursor.execute("""
            UPDATE attempts SET score = score - 5
            WHERE user_id=%s AND quiz_id=1
        """, (session['user_id'],))
        cursor.execute("""
            UPDATE users SET total_score = Greatest(total_score - 5,0)
            WHERE user_id=%s
            """,(session["user_id"],))
        db.commit()
